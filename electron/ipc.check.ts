...
for (const [channel, handler, service] of [
  ...
  assert.match(
    source,
    new RegExp(
      `ipcMain\\.handle\\(\\s*"${channel}",\\s*dataIssueHandlers\\.${handler},?\\s*\\)`,
    ),
  );
}
...
const services: DataIssueDesktopService = {
  list: () => {
    calls.push({ operation: "list", payload: undefined });
    return [];
  },
  create: (input) => {
    calls.push({ operation: "create", payload: input });
    return undefined as never;
  },
  load: (dataIssueId) => {
    calls.push({ operation: "load", payload: dataIssueId });
    return undefined as never;
  },
  startDiagnosis: (dataIssueId) => {
    calls.push({ operation: "startDiagnosis", payload: dataIssueId });
    return undefined as never;
  },
  previewExclusion: (input) => {
    calls.push({ operation: "previewExclusion", payload: input });
    return undefined as never;
  },
  confirmExclusion: (input) => {
    calls.push({ operation: "confirmExclusion", payload: input });
    return undefined as never;
  },
  previewRestore: (dataIssueId) => {
    calls.push({ operation: "previewRestore", payload: dataIssueId });
    return undefined as never;
  },
  confirmRestore: (input) => {
    calls.push({ operation: "confirmRestore", payload: input });
    return undefined as never;
  },
};
...
const fictionalCreate = {
  account: {
    ...
    kind: "loan" as const,
    typeLabel: "信貸",
    amountLines: [{ currency: "TWD", value: 123_456 }],
...
const fictionalExclusion = {
  ...
  sourceVersion: {
    sourceFileId: "source-fictional",
    importRunId: "run-fictional",
  },
};
...
const fictionalRestore = {
  dataIssueId: "issue-fictional",
  previewToken: "synthetic-restore-token",
};

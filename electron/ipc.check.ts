...
assert.match(source, /createAgentIpcHandlers\(agentService\)/);
assert.match(source, /ipcMain\.handle\("agent:v1:start", agentHandlers\.start\)/);
assert.match(source, /ipcMain\.handle\("agent:v1:status", agentHandlers\.status\)/);
assert.match(source, /ipcMain\.handle\("agent:v1:cancel", agentHandlers\.cancel\)/);
...
assert.match(mainSource, /createAgentHarnessService/);
assert.match(mainSource, /agentService/);

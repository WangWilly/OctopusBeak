...
export function refreshCanonicalEnrichmentProjection(
  ...
  const selectCurrent = db.prepare(`
    SELECT assertion.assertion_id, assertion.transaction_id, assertion.field_name,
     ...
     WHERE assertion.transaction_id = ?
       ...
       AND assertion.created_commit_id IN (
         SELECT commit_id FROM canonical_commits WHERE commit_sequence <= ?
       )
     ...
     ORDER BY (
       SELECT commit_sequence FROM canonical_commits WHERE commit_id = assertion.created_commit_id
     ) DESC, assertion.assertion_id DESC
     LIMIT 1
  `);
  ...
  for (const row of affected) {
    ...
    deleteCurrent.run(transactionId);
...
export function rebuildCanonicalEnrichmentProjection(
  ...
  const selectCurrent = db.prepare(`
    SELECT assertion.assertion_id, assertion.transaction_id, assertion.field_name,
     ...
     ORDER BY (SELECT commit_sequence FROM canonical_commits WHERE commit_id = assertion.created_commit_id) DESC, assertion.assertion_id DESC LIMIT 1
  `);
  const insert = db.prepare(`INSERT INTO current_transaction_enrichment(
      transaction_id, field_name, assertion_id, value_text, origin, producer_id,
      producer_version, route_id, taxonomy_id, taxonomy_version,
      taxonomy_dimension, taxonomy_code, projection_commit_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  for (const transaction of affected) {
    const transactionId = transaction.transaction_id as Uint8Array;
    for (const field of ENRICHMENT_FIELDS) {
      const row = selectCurrent.get(transactionId, field, cutoffSequence, cutoffSequence, cutoffSequence, cutoffSequence) as Record<string, unknown> | undefined;
      if (!row) continue;
      insert.run(transactionId, field, sqliteValue(row.assertion_id), sqliteValue(row.value_text), sqliteValue(row.origin), sqliteValue(row.producer_id), sqliteValue(row.producer_version), sqliteValue(row.route_id), sqliteValue(row.taxonomy_id ?? "transaction-taxonomy"), sqliteValue(row.taxonomy_version ?? "v1"), sqliteValue(row.taxonomy_dimension ?? null), sqliteValue(row.taxonomy_code ?? null), commitId);
    }
  }
}

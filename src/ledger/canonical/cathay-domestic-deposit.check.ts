...
const workflowSource = readFileSync(
  new URL("../../workflows/cathay-statements.ts", import.meta.url),
  "utf8",
);
...
function seedLegacyDatabase(directory: string, version: 1 | 2): void {
  ...
  ).run(1782864000000000);
  db.prepare(
    "INSERT INTO canonical_commits(commit_id, commit_sequence, recorded_at_utc_us, authority_route) VALUES (?, ?, ?, ?)",
  ).run(commitId, 1, 1782864000000000, route);
  db.prepare(
  ...
  ).run(route, "cathay", "domestic-deposit", "v1", commitId);
  db.prepare(
    "INSERT INTO source_connections(source_connection_id, integration_namespace, source_connection_key, created_commit_id) VALUES (?, ?, ?, ?)",
  ).run(connectionId, "cathay", "synthetic-legacy-connection", commitId);
  db.prepare(
    "INSERT INTO identity_epochs(identity_epoch_id, source_connection_id, epoch_key, created_commit_id) VALUES (?, ?, ?, ?)",
  ).run(epochId, connectionId, "synthetic-legacy-epoch", commitId);
  db.prepare(
    "INSERT INTO source_captures(capture_id, source_connection_id, identity_epoch_id, authority_route, stream, account_no, observed_at, scope_start, scope_end, completeness, commit_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
  ).run(
    captureId,
    connectionId,
    epochId,
  ...
  db.prepare(
    "INSERT INTO source_records(source_record_id, capture_id, commit_id, sequence_lexeme, payload_json) VALUES (?, ?, ?, ?, ?)",
  ).run(recordId, captureId, commitId, "1", '{"synthetic":true}');
  db.prepare(
  ...
  ).run(transactionId, accountId, "1", commitId);
  db.prepare(
    "INSERT INTO transaction_revisions(revision_id, transaction_id, source_record_id, capture_id, commit_id, revision_number, amount_coefficient, amount_scale, currency, direction, posting_status, effective_on, transaction_date_time_local, time_zone, time_precision, time_origin, utc_instant_utc_us) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
  ).run(
    revisionId,
    transactionId,
    recordId,
  ...
  db.prepare(
    "INSERT INTO source_assertions(assertion_id, transaction_id, revision_id, source_record_id, commit_id) VALUES (?, ?, ?, ?, ?)",
  ).run(assertionId, transactionId, revisionId, recordId, commitId);
  db.prepare(
    "INSERT INTO assertion_provenance(assertion_id, source_record_id, commit_id) VALUES (?, ?, ?)",
  ).run(assertionId, recordId, commitId);
  db.prepare(
    "INSERT INTO source_sync_states(source_connection_id, account_id, stream, scope_start, scope_end, cursor, last_capture_id, commit_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
  ).run(
    connectionId,
    accountId,
    "domestic-deposit",
  ...
  db.prepare(
    "INSERT INTO current_transactions(transaction_id, revision_id, commit_id) VALUES (?, ?, ?)",
  ).run(transactionId, revisionId, commitId);
  if (version === 2) {
    db.exec(
    ...
    db.exec(
      "CREATE TABLE current_projection_state (generation INTEGER PRIMARY KEY CHECK(generation = 1), commit_id BLOB NOT NULL REFERENCES canonical_commits(commit_id))",
    );
    db.prepare(
...
function seedLegacyV5Database(directory: string): void {
  ...
  db.prepare(
    "UPDATE current_transactions SET projection_commit_id = commit_id, revision_commit_id = commit_id",
  ).run();
  ...
  `);
  db.exec(
    "CREATE UNIQUE INDEX source_records_source_record_capture ON source_records(source_record_id, capture_id)",
  );
  db.prepare(
    ...
    "bounded-range",
    "complete-range",
    "success-status-scope-count-details",
  ...
  ).run(legacyId(5), legacyId(12), legacyId(4), legacyId(6), "1", legacyId(1));
  db.prepare(
    "INSERT INTO assertion_lifecycle_events(event_id, assertion_id, transaction_id, revision_id, capture_id, scope_id, commit_id, event_kind) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
  ).run(
    legacyId(14),
    legacyId(9),
    legacyId(7),
  ...
  db.exec(
    "CREATE INDEX idx_capture_scopes_account_time ON capture_scopes(source_connection_id, identity_epoch_id, account_id, scope_start, scope_end, commit_id)",
  );
  db.exec(
    "CREATE INDEX idx_capture_scope_pages_proof ON capture_scope_pages(scope_id, page_ordinal, commit_id)",
  );
  db.exec(
  ...
  db.exec(
    "CREATE INDEX idx_assertion_lifecycle_assertion_knowledge ON assertion_lifecycle_events(assertion_id, commit_id, event_kind, event_id)",
  );
  db.exec(
    "CREATE INDEX idx_assertion_lifecycle_transaction_knowledge ON assertion_lifecycle_events(transaction_id, commit_id, event_kind, event_id)",
  );
  db.exec(
    "CREATE INDEX idx_assertion_lifecycle_scope ON assertion_lifecycle_events(scope_id, commit_id, assertion_id)",
  );
  db.exec(
  ...
  ).run(3, 1782864000000002);
  db.prepare(
    "INSERT INTO schema_migrations(version, applied_at_utc_us) VALUES (?, ?)",
  ).run(4, 1782864000000003);
  db.prepare(
    "INSERT INTO schema_migrations(version, applied_at_utc_us) VALUES (?, ?)",
  ).run(5, 1782864000000004);
  db.exec("PRAGMA user_version = 5");
  db.close();
}

const ledgerDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-"),
);
try {
  ...
  assert.deepEqual(first.transactions[0]?.amount, {
    coefficient: "12500",
    scale: 0,
  });
  ...
  try {
    ...
    const commitRow = db
      .prepare("SELECT commit_id, recorded_at_utc_us FROM canonical_commits")
    ...
    assert.equal(
      db.prepare("SELECT commit_kind FROM canonical_commits").get()
        ?.commit_kind,
      "source_capture",
    );
    const captureRow = db
      .prepare("SELECT capture_id, commit_id FROM source_captures")
      .get() as Record<string, unknown>;
    ...
    assert.equal(
      revisionColumns.some((column) => column.name === "balance_coefficient"),
      false,
    );
    assert.equal(
      revisionColumns.some((column) => column.name === "balance_scale"),
      false,
    );
    assert.equal(
      db
        .prepare(
          "SELECT COUNT(*) AS count FROM current_transactions WHERE commit_id IS NOT NULL",
        )
        .get()?.count,
      3,
    );
    assert.equal(
      db.prepare("SELECT cursor FROM source_sync_states").get()?.cursor,
      null,
    );
    assert.deepEqual(
      {
        ...(db
          .prepare(
            "SELECT completeness, completeness_basis, completeness_rule_version FROM source_captures",
          )
          .get() as Record<string, unknown>),
      },
      {
    ...
    assert.deepEqual(
      (
        db
          .prepare(
            "SELECT role, local_value, time_precision, time_origin FROM transaction_time_observations ORDER BY role, local_value LIMIT 2",
          )
      ...
      [
        {
          role: "accounting",
          local_value: "2026-07-01",
          time_precision: "date",
          time_origin: "source_reported",
        },
        {
          role: "accounting",
          local_value: "2026-07-02",
          time_precision: "date",
          time_origin: "source_reported",
        },
      ],
  ...
  assert.equal(
    current.transactions[0]?.postingOrigin,
    CATHAY_POSTING_MAPPING.origin,
  );
  assert.equal(
    current.transactions[0]?.postingBasis,
    CATHAY_POSTING_MAPPING.basis,
  );
  assert.equal(
    current.transactions[0]?.postingRuleVersion,
    CATHAY_POSTING_MAPPING.ruleVersion,
  );
  assert.equal(
    current.transactions[0]?.displayLabel,
  ...
  assert.equal(
    current.transactions[0]?.utcInstantUtcUs,
    new Date("2026-07-01T09:00:00+08:00").getTime() * 1000,
  );
  ...
  const historicalBeforeCommit = await query.historical({
    ...
    cutoff: {
      ...
      knowledgeAt: String(first.commitSequence - 1),
    },
  });
  ...
  const historical = await query.historical({
    ...
    cutoff: {
      kind: "both",
      financialAt: "2026-12-31",
      knowledgeAt: String(first.commitSequence),
    },
  });
  ...
  assert.equal(
    lineage.entries[0]?.revision.transactionId,
    current.transactions[0]?.id,
  );
  assert.equal(
    lineage.entries[0]?.assertion.revisionId,
    lineage.entries[0]?.revision.id,
  );
  assert.equal(
    lineage.entries[0]?.sourceRecord.captureId,
    lineage.entries[0]?.capture.id,
  );
  assert.equal(
    lineage.entries[0]?.capture.authorityRoute,
    CATHAY_DOMESTIC_DEPOSIT_FIXTURE.authorityRoute,
  );
  assert.equal(
    lineage.entries[0]?.sourceRecord.description,
    "Synthetic Cathay deposit description",
  );
  assert.equal(
    lineage.entries[0]?.sourceRecord.scopeProof.accountNo,
    CATHAY_DOMESTIC_DEPOSIT_FIXTURE.accountNo,
  );
  assert.equal(
    lineage.entries[0]?.sourceRecord.scopeProof.completeness,
    "complete-range",
  );
  ...
  assert.equal(
    repeated.transactions.filter((transaction) => transaction.revisionCreated)
      .length,
    0,
  );
  ...
  try {
    ...
    assert.equal(
      repeatDb.prepare("SELECT COUNT(*) AS count FROM source_records").get()
        ?.count,
      6,
    );
    assert.equal(
    ...
    assert.equal(
      repeatDb.prepare("SELECT COUNT(*) AS count FROM source_assertions").get()
        ?.count,
      3,
    );
    assert.equal(
  ...
  const identityRows = sourceIdentityRepair
    ...
    WHERE sr.sequence_lexeme = '1' ORDER BY c.commit_sequence`,
    )
    .all() as Array<{ source_record_id?: Uint8Array }>;
  ...
  const immutableRevisionRecord = sourceIdentityRepair
    .prepare(
      `SELECT source_record_id FROM transaction_revisions
    WHERE transaction_id = ?`,
    )
    .get(
      Buffer.from(
        first.transactions[0]!.transactionId.replaceAll("-", ""),
        "hex",
      ),
    )?.source_record_id as Uint8Array;
  assert.deepEqual(
    Buffer.from(immutableRevisionRecord),
    Buffer.from(firstObservedRecord),
  );
  ...
  for (const table of [
    ...
    sourceIdentityRepair
      .prepare(
        `UPDATE ${table} SET source_record_id = ? WHERE source_record_id = ?`,
      )
      .run(highRevision, firstObservedRecord);
    sourceIdentityRepair
      .prepare(
        `UPDATE ${table} SET source_record_id = ? WHERE source_record_id = ?`,
      )
  ...
  try {
    const sourceAssertion = sourceIdentityAfter
      .prepare(
        "SELECT source_record_id FROM source_assertions WHERE transaction_id = ?",
      )
      .get(
        Buffer.from(
          first.transactions[0]!.transactionId.replaceAll("-", ""),
      ...
      ) as { source_record_id?: Uint8Array };
    assert.deepEqual(
      Buffer.from(sourceAssertion.source_record_id!),
      highRevision,
    );
    sourceIdentityAfter.close();
    const sourceLineage = await query.lineage({
      kind: "lineage",
      subject: {
        kind: "transaction",
        id: first.transactions[0]!.transactionId,
      },
    });
    const sourceEntry = sourceLineage.entries[0]!;
    assert.deepEqual(
      Buffer.from(sourceEntry.sourceRecord.id.replaceAll("-", ""), "hex"),
      highRevision,
    );
    ...
    assert.deepEqual(
      sourceEntry.provenance.map((item) => item.sourceRecordId).sort(),
      [lowOccurrence, highRevision].map(sourceIdText).sort(),
    );
  } finally {
    sourceIdentityAfter.close();
  }
  ...
  assert.equal(
    (await boundaryQuery.current({ kind: "current" })).transactions.length,
    3,
  );
  assert.equal(
    (
      await boundaryQuery.historical({
        kind: "historical",
        cutoff: {
          kind: "both",
          financialAt: "2026-12-31",
          knowledgeAt: String(repeated.commitSequence),
        },
      })
    ).transactions.length,
    3,
...
const multiScopeDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-sync-scopes-"),
);
try {
  const secondAccount = "SYNTHETIC-ACCOUNT-002";
  const secondRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
    "SYNTHETIC-ACCOUNT-001",
    secondAccount,
  );
  const firstCommit = await commitCathayDomesticDepositSync(
    multiScopeDir,
  ...
  try {
    ...
    assert.equal(
      multiDb.prepare("SELECT COUNT(*) AS count FROM capture_scope_pages").get()
        ?.count,
      2,
    );
    assert.equal(
      multiDb
        .prepare("SELECT COUNT(*) AS count FROM source_record_scopes")
        .get()?.count,
      6,
    );
    assert.equal(
      multiDb
        .prepare(
          "SELECT COUNT(*) AS count FROM source_record_scopes WHERE sequence_lexeme IN ('1', '2', '3')",
        )
        .get()?.count,
      6,
    );
    ...
    assert.equal(
      multiDb
        .prepare(
          "SELECT COUNT(*) AS count FROM source_records WHERE sequence_lexeme LIKE '%:%'",
        )
        .get()?.count,
      0,
    );
    assert.equal(
      multiDb.prepare("SELECT COUNT(*) AS count FROM source_sync_states").get()
        ?.count,
      2,
    );
    assert.equal(
      multiDb
        .prepare(
          "SELECT COUNT(DISTINCT commit_id) AS count FROM source_sync_states",
        )
        .get()?.count,
      1,
    );
    assert.equal(
      multiDb
        .prepare(
          "SELECT COUNT(*) AS count FROM source_sync_states WHERE cursor IS NOT NULL",
        )
        .get()?.count,
      0,
    );
    ...
    assert.equal(
      multiDb
        .prepare(
          "SELECT 1 FROM sqlite_master WHERE name = 'transport_checkpoints'",
        )
        .get(),
      undefined,
    );
  ...
  try {
    ...
    assert.equal(
      repeatedDb.prepare("SELECT COUNT(*) AS count FROM source_records").get()
        ?.count,
      12,
    );
    assert.equal(
  ...
  await rm(multiScopeDir, { recursive: true, force: true });
}

const populatedV4ScopeMigrationDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-migration-v4-scopes-"),
);
try {
  const secondAccount = "SYNTHETIC-ACCOUNT-002";
  const secondRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
  ...
  try {
    assert.equal(
      Number(migratedV4.prepare("PRAGMA user_version").get()?.user_version),
      CANONICAL_SCHEMA_VERSION,
    );
    assert.equal(
      migratedV4
    ...
    assert.equal(
      migratedV4
        .prepare(
          "SELECT COUNT(DISTINCT sequence_lexeme) AS count FROM source_records",
        )
        .get()?.count,
      3,
    );
  ...
  await rm(populatedV4ScopeMigrationDir, { recursive: true, force: true });
}

const provenanceOnlyV4MigrationDir = await mkdtemp(
  join(
    process.env.TMPDIR ?? "/tmp",
    "cathay-canonical-migration-v4-provenance-only-",
  ),
);
try {
  await commitCathayDomesticDepositSync(
  ...
  await commitCathayDomesticDepositSync(provenanceOnlyV4MigrationDir, {
    ...syncInput(),
    observedAt: "2026-08-18T00:00:00+08:00",
  });
  ...
  try {
    ...
    provenanceOnlyV4Migrated.close();
  }
} finally {
  await rm(provenanceOnlyV4MigrationDir, { recursive: true, force: true });
}

const restorationV4MigrationDir = await mkdtemp(
  join(
    process.env.TMPDIR ?? "/tmp",
    "cathay-canonical-migration-v4-restoration-",
  ),
);
try {
  await commitCathayDomesticDepositSync(restorationV4MigrationDir, syncInput());
  const emptyRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
    /"count":3,"startDate":"2025-08-17","endDate":"2026-08-17","details":\[[\s\S]*?\]\}/,
    '"count":0,"startDate":"2025-08-17","endDate":"2026-08-17","details":[]}',
  );
  await commitCathayDomesticDepositSync(restorationV4MigrationDir, {
    ...syncInput([
      syncPage(CATHAY_DOMESTIC_DEPOSIT_FIXTURE.accountNo, emptyRaw),
    ]),
    observedAt: "2026-08-18T00:00:00+08:00",
  });
  const restoration = await commitCathayDomesticDepositSync(
  ...
  try {
    assert.equal(
      Number(failedV4.prepare("PRAGMA user_version").get()?.user_version),
      4,
    );
    assert.equal(
      failedV4.prepare("SELECT COUNT(*) AS count FROM source_records").get()
        ?.count,
      6,
    );
    assert.equal(
      failedV4
        .prepare(
          "SELECT 1 FROM sqlite_master WHERE name = 'source_record_scopes'",
        )
        .get(),
      undefined,
    );
  ...
  try {
    const migratedProjection = restoredMigrationDb
      .prepare(
        "SELECT commit_id, projection_commit_id, revision_commit_id FROM current_transactions LIMIT 1",
      )
      .get() as Record<string, unknown>;
    const migratedState = restoredMigrationDb
      .prepare(
  ...
  await rm(restorationV4MigrationDir, { recursive: true, force: true });
}

const tombstoneDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-tombstone-"),
);
try {
  assert.throws(
    () =>
  ...
  await rm(tombstoneDir, { recursive: true, force: true });
}

const incomparableDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-incomparable-"),
);
try {
  ...
  const incomparableQuery =
    createCathayCanonicalFinancialQuery(incomparableDir);
  assert.equal(
    (await incomparableQuery.current({ kind: "current" })).transactions.length,
    3,
  );
} finally {
  await rm(incomparableDir, { recursive: true, force: true });
}
...
for (const [label, pages, expected] of [
  ...
  [
    ...
    [{ ...syncPage(), nextPageToken: "synthetic-next-token" }] as const,
  ...
  [
    ...
    [
      ...
      {
        ...syncPage(
          "SYNTHETIC-ACCOUNT-002",
          CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
            "SYNTHETIC-ACCOUNT-001",
            "SYNTHETIC-ACCOUNT-002",
          ),
        ),
        contractFingerprint: "synthetic-drift",
      },
    ] as const,
    /unsupported|contract or preflight fingerprint drifted across scopes/,
  ],
] as const) {
  const rejectedSyncDir = await mkdtemp(
    join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-sync-reject-"),
  );
  try {
    assert.throws(
      () =>
        commitCathayDomesticDepositSync(rejectedSyncDir, syncInput([...pages])),
      expected,
      label,
    );
    ...
    try {
      ...
      rejectedSyncDb.close();
    }
  } finally {
    await rm(rejectedSyncDir, { recursive: true, force: true });
  }
}

const lifecycleDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-lifecycle-"),
);
try {
  const first = await commitCathayDomesticDepositSync(
    lifecycleDir,
    syncInput(),
  );
  const emptyRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
  ...
  const historicalWithdrawn = await withdrawnQuery.historical({
    kind: "historical",
    cutoff: {
      kind: "both",
      financialAt: "2026-12-31",
      knowledgeAt: String(withdrawn.commitSequence),
    },
  });
  ...
  assert.equal(
    (await withdrawnQuery.current({ kind: "current" })).transactions.length,
    3,
  );
  assert.equal(
  ...
  try {
    const projection = restoredProjectionDb
      .prepare(
        "SELECT commit_id, projection_commit_id, revision_commit_id FROM current_transactions LIMIT 1",
      )
      .get() as Record<string, unknown>;
    const projectionState = restoredProjectionDb
      .prepare(
        "SELECT commit_id FROM current_projection_state WHERE generation = 1",
      )
      .get() as Record<string, unknown>;
    ...
    restoredProjectionDb.close();
  }
  assert.equal(
    (
      await withdrawnQuery.historical({
        kind: "historical",
        cutoff: {
          kind: "both",
          financialAt: "2026-12-31",
          knowledgeAt: String(restored.commitSequence),
        },
      })
  ...
  const lifecycleKinds = restoredLineage.entries.flatMap((entry) =>
    entry.lifecycleEvents.map((event) => event.kind),
  );
  ...
  assert.equal(
    restoredLineage.entries[0]?.revision.administrativeState,
    "active",
  );

  const incompleteRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
    '"count":3',
    '"count":2',
  );
  assert.throws(
    () =>
      commitCathayDomesticDepositSync(lifecycleDir, {
        ...syncInput([
          syncPage(CATHAY_DOMESTIC_DEPOSIT_FIXTURE.accountNo, incompleteRaw),
        ]),
  ...
  const changedRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
    '"incomeAmt":12500',
    '"incomeAmt":13000',
  ).replace('"balance":12500', '"balance":13000');
  const changed = await commitCathayDomesticDepositSync(lifecycleDir, {
    ...syncInput([
      syncPage(CATHAY_DOMESTIC_DEPOSIT_FIXTURE.accountNo, changedRaw),
    ]),
  ...
  try {
    ...
    assert.equal(
      changedDb
        .prepare(
          "SELECT COUNT(*) AS count FROM assertion_lifecycle_events WHERE event_kind = 'superseded'",
        )
        .get()?.count,
      1,
    );
...
const semanticDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-semantics-"),
);
try {
  const seeded = await commitCathayDomesticDepositSync(
    semanticDir,
    syncInput(),
  );
  ...
  semanticDb
    ...
    .run(CATHAY_DOMESTIC_DEPOSIT_AUTHORITY);
  semanticDb.close();
  const semanticQuery = createCathayCanonicalFinancialQuery(semanticDir);
  const currentSemantic = (await semanticQuery.current({ kind: "current" }))
    .transactions[0]!;
  ...
  const semanticLineage = await semanticQuery.lineage({
    kind: "lineage",
    subject: { kind: "transaction", id: seeded.transactions[0]!.transactionId },
  });
  assert.equal(semanticLineage.entries[0]?.revision.economicStatus, "refund");
  assert.equal(
    semanticLineage.entries[0]?.revision.administrativeState,
    "deleted",
  );
  const emptyRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
    /"count":3,"startDate":"2025-08-17","endDate":"2026-08-17","details":\[[\s\S]*?\]\}/,
  ...
  const withdrawnSemanticRow = (
    await semanticQuery.historical({
      kind: "historical",
      cutoff: {
        kind: "both",
        financialAt: "2026-12-31",
        knowledgeAt: String(withdrawnSemantic.commitSequence),
      },
    })
  ).transactions[0]!;
  ...
  await commitCathayDomesticDepositSync(semanticDir, {
    ...syncInput(),
    observedAt: "2026-08-20T00:00:00+08:00",
  });
  const restoredSemantic = (await semanticQuery.current({ kind: "current" }))
    .transactions[0]!;
...
const derivedDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-derived-v6-"),
);
try {
  ...
  const firstDerived = await commitCathayDerivedImportRun(
    derivedDir,
    derivedInput,
  );
  const currentDerived = (
    await createCathayCanonicalFinancialQuery(derivedDir).current({
      kind: "current",
    })
  ).transactions[0]!;
  ...
  try {
    assert.equal(
      sharedSpineAfterFirst
        .prepare(
          "SELECT COUNT(*) AS count FROM assertions WHERE origin = 'source'",
        )
        .get()?.count,
    ...
    assert.equal(
      sharedSpineAfterFirst
        .prepare(
          "SELECT COUNT(*) AS count FROM assertion_provenance WHERE run_id IS NOT NULL",
        )
        .get()?.count,
      1,
    );
  ...
  const sharedSpineBeforeUnchangedCounts = {
    transitions: sharedSpineBeforeUnchanged
      .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
      .get()?.count,
    provenance: sharedSpineBeforeUnchanged
      .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
      .get()?.count,
  };
  sharedSpineBeforeUnchanged.close();
  const unchanged = await commitCathayDerivedImportRun(
    derivedDir,
    derivedInput,
  );
  ...
  try {
    assert.equal(
      sharedSpineAfterUnchanged
        .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
        .get()?.count,
      sharedSpineBeforeUnchangedCounts.transitions,
    );
    assert.equal(
      sharedSpineAfterUnchanged
        .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
        .get()?.count,
      Number(sharedSpineBeforeUnchangedCounts.provenance) + 1,
    );
  } finally {
    sharedSpineAfterUnchanged.close();
  }
  const changed = await commitCathayDerivedImportRun(derivedDir, {
    ...derivedInput,
    scope: [
      {
        transactionId,
  ...
  const beforeRuleLineageCounts = {
    commits: beforeRuleLineage
      .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
      .get()?.count,
    runs: beforeRuleLineage
      .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
      .get()?.count,
    assertions: beforeRuleLineage
      .prepare("SELECT COUNT(*) AS count FROM derived_assertions")
      .get()?.count,
    provenance: beforeRuleLineage
      .prepare("SELECT COUNT(*) AS count FROM derived_assertion_provenance")
      .get()?.count,
    current: beforeRuleLineage
      .prepare(
        "SELECT value_text, projection_commit_id FROM current_transaction_fields WHERE transaction_id = ? AND field_name = 'display_name'",
      )
      .get(Buffer.from(transactionId.replaceAll("-", ""), "hex")),
  };
  beforeRuleLineage.close();
  const ruleLineageDiagnostic = await runCathayDerivedImportRun(derivedDir, {
    ...derivedInput,
  ...
  try {
    assert.equal(
      afterRuleLineage
        .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
        .get()?.count,
      beforeRuleLineageCounts.commits,
    );
    assert.equal(
      afterRuleLineage
        .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
        .get()?.count,
      beforeRuleLineageCounts.runs,
    );
    assert.equal(
      afterRuleLineage
        .prepare("SELECT COUNT(*) AS count FROM derived_assertions")
        .get()?.count,
      beforeRuleLineageCounts.assertions,
    );
    assert.equal(
      afterRuleLineage
        .prepare("SELECT COUNT(*) AS count FROM derived_assertion_provenance")
        .get()?.count,
      beforeRuleLineageCounts.provenance,
    );
    assert.deepEqual(
      afterRuleLineage
        .prepare(
        ...
        .get(Buffer.from(transactionId.replaceAll("-", ""), "hex")),
      beforeRuleLineageCounts.current,
    );
  } finally {
    afterRuleLineage.close();
  }
  const producerDiagnostic = await runCathayDerivedImportRun(derivedDir, {
    ...derivedInput,
  ...
  const withdrawn = await commitCathayDerivedImportRun(derivedDir, {
    ...derivedInput,
    scope: [
      {
        transactionId,
  ...
  const unsupportedRetryTransitions = unsupportedRetryBefore
    .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
    .get()?.count;
  const unsupportedRetryProvenance = unsupportedRetryBefore
    .prepare(
      "SELECT COUNT(*) AS count FROM assertion_provenance WHERE run_id IS NOT NULL",
    )
    .get()?.count;
  unsupportedRetryBefore.close();
  const unsupportedRetry = await commitCathayDerivedImportRun(derivedDir, {
    ...derivedInput,
    scope: [
      {
        transactionId,
  ...
  try {
    assert.equal(
      unsupportedRetryAfter
        .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
        .get()?.count,
      unsupportedRetryTransitions,
    );
    assert.equal(
      unsupportedRetryAfter
        .prepare(
        ...
        .get()?.count,
      Number(unsupportedRetryProvenance) + 2,
    );
  } finally {
  ...
  const historicalLineageSnapshot = {
    commits: historicalLineageBefore
      .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
      .get()?.count,
    runs: historicalLineageBefore
      .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
      .get()?.count,
    assertions: historicalLineageBefore
      .prepare("SELECT COUNT(*) AS count FROM derived_assertions")
      .get()?.count,
    provenance: historicalLineageBefore
      .prepare("SELECT COUNT(*) AS count FROM derived_assertion_provenance")
      .get()?.count,
    lifecycle: historicalLineageBefore
      .prepare(
        "SELECT COUNT(*) AS count FROM derived_assertion_lifecycle_events",
      )
      .get()?.count,
    current: historicalLineageBefore
      .prepare(
        "SELECT value_text, origin, derived_assertion_id, user_assertion_id, projection_commit_id FROM current_transaction_fields WHERE transaction_id = ? AND field_name = 'display_name'",
      )
      .get(Buffer.from(transactionId.replaceAll("-", ""), "hex")),
    projection: historicalLineageBefore
  ...
  for (const mismatch of [
    ...
    const historicalLineageDiagnostic = await runCathayDerivedImportRun(
      derivedDir,
      {
        ...derivedInput,
        ...mismatch,
        ...
        scope: [
          {
            ...
            field: "display_name" as const,
            state: "supported" as const,
            value: "Historical mismatch label",
          },
        ],
      } as never,
    );
    ...
    try {
      assert.deepEqual(
        {
          commits: historicalLineageAfter
            .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
            .get()?.count,
          runs: historicalLineageAfter
            .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
            .get()?.count,
          assertions: historicalLineageAfter
            .prepare("SELECT COUNT(*) AS count FROM derived_assertions")
            .get()?.count,
          provenance: historicalLineageAfter
            .prepare(
              "SELECT COUNT(*) AS count FROM derived_assertion_provenance",
            )
            .get()?.count,
          lifecycle: historicalLineageAfter
            .prepare(
              "SELECT COUNT(*) AS count FROM derived_assertion_lifecycle_events",
            )
            .get()?.count,
          current: historicalLineageAfter
            .prepare(
              "SELECT value_text, origin, derived_assertion_id, user_assertion_id, projection_commit_id FROM current_transaction_fields WHERE transaction_id = ? AND field_name = 'display_name'",
            )
            .get(Buffer.from(transactionId.replaceAll("-", ""), "hex")),
          projection: historicalLineageAfter
            .prepare(
              "SELECT generation, commit_id FROM current_projection_state",
            )
            .all(),
        },
      ...
      historicalLineageAfter.close();
    }
  }
  const query = createCathayCanonicalFinancialQuery(derivedDir);
  assert.equal(
    (await query.current({ kind: "current" })).transactions[0]?.displayLabel,
    CATHAY_DOMESTIC_DEPOSIT_FIXTURE.rawResponse.includes(
      "Synthetic Cathay deposit description",
    )
      ? "Synthetic Cathay deposit description"
      : null,
  );
  const historicalChanged = await query.historical({
    kind: "historical",
    cutoff: {
      kind: "both",
      financialAt: "2026-12-31",
      knowledgeAt: String(changed.commitSequence),
    },
  });
  assert.equal(
    historicalChanged.transactions[0]?.displayLabel,
    "Changed label",
  );
  const lineage = await query.lineage({
    kind: "lineage",
    subject: { kind: "transaction", id: transactionId },
  });
  const derivedKinds = lineage.entries.flatMap((entry) =>
    entry.derivedAssertions.map((assertion) => assertion.state),
  );
  assert.equal(derivedKinds.includes("withdrawn"), true);
  assert.equal(
    lineage.entries[0]?.derivedAssertions.some(
      (assertion) => assertion.value === "Changed label",
    ),
    true,
  );
  assert.equal(
    lineage.entries[0]?.derivedAssertions.find(
      (assertion) => assertion.value === "Derived note",
    )?.provenance.length,
    3,
  );
  ...
  try {
    ...
    assert.equal(
      db
        .prepare(
          "SELECT COUNT(*) AS count FROM derived_assertion_provenance provenance JOIN derived_scope_coordinates coordinate ON coordinate.coordinate_id = provenance.coordinate_id WHERE coordinate.output_state = 'unsupported'",
        )
        .get()?.count,
      4,
    );
    assert.equal(
      db
        .prepare(
          "SELECT COUNT(*) AS count FROM derived_assertion_lifecycle_events WHERE event_kind = 'withdrawn'",
        )
        .get()?.count,
      2,
    );
  ...
  const user = await commitCathayUserAssertion(derivedDir, {
    transactionId,
    target: { kind: "transaction", id: transactionId, field: "display_name" },
    field: "display_name",
    value: "User label",
  });
  const userCurrent = (await query.current({ kind: "current" }))
    .transactions[0]!;
  ...
  const historicalUser = await query.historical({
    kind: "historical",
    cutoff: {
      kind: "both",
      financialAt: "2026-12-31",
      knowledgeAt: String(user.commitSequence),
    },
  });
  ...
  const repeatedDisplayCounts = {
    assertions: repeatedDisplayBefore
      .prepare("SELECT COUNT(*) AS count FROM assertions WHERE origin = 'user'")
      .get()?.count,
    transitions: repeatedDisplayBefore
      .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
      .get()?.count,
  };
  repeatedDisplayBefore.close();
  const repeatedDisplay = await commitCathayUserAssertion(derivedDir, {
    transactionId,
    field: "display_name",
    value: "User label",
  });
  const repeatedNoteFirst = await commitCathayUserAssertion(derivedDir, {
    transactionId,
    field: "note",
    value: "User note",
  });
  const repeatedNote = await commitCathayUserAssertion(derivedDir, {
    transactionId,
    field: "note",
    value: "User note",
  });
  ...
  try {
    assert.equal(
      repeatedUserDb
        .prepare(
          "SELECT COUNT(*) AS count FROM assertions WHERE origin = 'user'",
        )
        .get()?.count,
      Number(repeatedDisplayCounts.assertions) + 1,
    );
    assert.equal(
      repeatedUserDb
        .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
        .get()?.count,
      Number(repeatedDisplayCounts.transitions) + 1,
    );
    assert.equal(
      repeatedUserDb
        .prepare(
          "SELECT COUNT(*) AS count FROM assertion_provenance WHERE commit_id IN (SELECT commit_id FROM canonical_commits WHERE commit_sequence IN (?, ?))",
        )
        .get(repeatedDisplay.commitSequence, repeatedNote.commitSequence)
        ?.count,
      2,
    );
    assert.equal(
      repeatedUserDb
        .prepare(
          "SELECT COUNT(*) AS count FROM assertion_transitions WHERE commit_id IN (SELECT commit_id FROM canonical_commits WHERE commit_sequence IN (?, ?))",
        )
        .get(repeatedDisplay.commitSequence, repeatedNote.commitSequence)
        ?.count,
      0,
    ...
    repeatedUserDb.close();
  }
  assert.equal(
    (await query.current({ kind: "current" })).transactions[0]?.note,
    "User note",
  );
  assert.equal(
    (
      await query.historical({
        ...
        cutoff: {
          kind: "both",
          financialAt: "2026-12-31",
          knowledgeAt: String(repeatedNote.commitSequence),
        },
      })
    ).transactions[0]?.note,
    "User note",
  ...
  const reobservation = tamperDb
    ...
    ORDER BY commit_row.commit_sequence DESC LIMIT 1`,
    )
    .get() as { assertion_id?: Uint8Array; commit_id?: Uint8Array };
  const sourceAssertionId = Buffer.from(
    (
      tamperDb
        .prepare(
  ...
  const originalRoute = (
    tamperDb
      .prepare(
        "SELECT authority_route FROM canonical_commits WHERE commit_id = ?",
      )
      .get(reobservationCommitId) as { authority_route?: string }
  ).authority_route!;
  const originalAssertion = reobservationAssertionId;
  const assertTamperedRecovery = async (
    label: string,
    mutate: () => void,
    restore: () => void,
  ): Promise<void> => {
    ...
    try {
      ...
      await assert.rejects(
        () => rebuildCathayCanonicalProjection(derivedDir),
        /provenance|authority|evidence|route|coordinate/i,
        label,
      );
    } finally {
  ...
  await assertTamperedRecovery(
    "provenance-only wrong route",
    () => {
      tamperDb
        .prepare(
          "UPDATE canonical_commits SET authority_route = 'evil/route' WHERE commit_id = ?",
        )
        .run(reobservationCommitId);
    },
    () => {
      tamperDb
        .prepare(
          "UPDATE canonical_commits SET authority_route = ? WHERE commit_id = ?",
        )
        .run(originalRoute, reobservationCommitId);
    },
  );
  await assertTamperedRecovery(
    "provenance-only wrong assertion",
    () => {
      ...
      tamperDb
        ...
        .run(
          sourceAssertionId,
          reobservationAssertionId,
          reobservationCommitId,
        );
    },
    () => {
      tamperDb
  ...
  await assertTamperedRecovery(
    "provenance-only orphan commit",
    () => {
      const sourceCommit = Buffer.from(
        (
          tamperDb
            .prepare("SELECT commit_id FROM source_captures LIMIT 1")
            .get() as { commit_id?: Uint8Array }
        ).commit_id!,
      );
      tamperDb
        .prepare(
          "UPDATE assertion_provenance SET commit_id = ? WHERE assertion_id = ? AND commit_id = ?",
        )
        .run(sourceCommit, reobservationAssertionId, reobservationCommitId);
    },
    () => {
      tamperDb
        .prepare(
          "UPDATE assertion_provenance SET commit_id = ? WHERE assertion_id = ? AND commit_id = (SELECT commit_id FROM source_captures LIMIT 1)",
        )
        .run(reobservationCommitId, reobservationAssertionId);
    },
  );
  tamperDb.close();
  await commitCathayUserAssertion(derivedDir, {
    transactionId,
    field: "display_name",
    value: null,
  });
  ...
  try {
    ...
    const compatibilityAssertionCounts = {
      ...
      derived: authorityDb
        ...
        .get()?.count,
      user: authorityDb
        .prepare("SELECT COUNT(*) AS count FROM user_assertions")
        .get()?.count,
    };
    assert.throws(
      () =>
        authorityDb.exec("UPDATE source_assertions SET commit_id = commit_id"),
      /cannot modify source_assertions because it is a view/,
    );
    assert.throws(
      () =>
        authorityDb.exec(
          "UPDATE derived_assertions SET value_text = 'tampered'",
        ),
      /cannot modify derived_assertions because it is a view/,
    );
    assert.throws(
      () =>
        authorityDb.exec("UPDATE user_assertions SET value_text = 'tampered'"),
      /cannot modify user_assertions because it is a view/,
    );
    assert.deepEqual(
      {
        source: authorityDb
    ...
    const sharedEventIds = (
      authorityDb
        .prepare("SELECT event_id FROM assertion_transitions ORDER BY event_id")
        .all() as Array<Record<string, unknown>>
    ).map((row) => Buffer.from(row.event_id as Uint8Array).toString("hex"));
    const compatibilityEventIds = (
      authorityDb
        .prepare(
          `SELECT event_id FROM assertion_lifecycle_events
      UNION SELECT event_id FROM derived_assertion_lifecycle_events
      UNION SELECT event_id FROM user_assertion_lifecycle_events ORDER BY event_id`,
        )
        .all() as Array<Record<string, unknown>>
    ).map((row) => Buffer.from(row.event_id as Uint8Array).toString("hex"));
    ...
    assert.equal(
      authorityDb
        .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
        .get()?.count,
      new Set(sharedEventIds).size,
    );
  } finally {
    authorityDb.close();
  }
  const userLineage = await query.lineage({
    kind: "lineage",
    subject: { kind: "transaction", id: transactionId },
  });
  assert.equal(
    userLineage.entries[0]?.userAssertions.some(
      (assertion) => assertion.state === "withdrawn",
    ),
    true,
  );
  ...
  sharedMetadataEdit
    ...
    .run("Shared authority label");
  sharedMetadataEdit.close();
  const sharedAuthorityLineage = await query.lineage({
    kind: "lineage",
    subject: { kind: "transaction", id: transactionId },
  });
  assert.equal(
    sharedAuthorityLineage.entries[0]?.derivedAssertions.some(
      (assertion) => assertion.value === "Shared authority label",
    ),
    true,
  );
  ...
  try {
    const sourceAssertionId = originTriggerDb
      ...
      .get()?.assertion_id as Uint8Array;
    assert.throws(
      () =>
        originTriggerDb
          .prepare(
            "UPDATE current_transaction_fields SET origin = 'derived', derived_assertion_id = ?, user_assertion_id = NULL WHERE transaction_id = ? AND field_name = 'display_name'",
          )
          .run(
            sourceAssertionId,
            Buffer.from(transactionId.replaceAll("-", ""), "hex"),
          ),
      /origin mismatch/,
    );
  } finally {
    originTriggerDb.close();
  }
  await commitCathayUserAssertion(derivedDir, {
    transactionId,
    field: "display_name",
  ...
  const malformedTargetSnapshot = {
    commits: malformedTargetBefore
      .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
      .get()?.count,
    userAssertions: malformedTargetBefore
      .prepare("SELECT COUNT(*) AS count FROM assertions WHERE origin = 'user'")
      .get()?.count,
    transitions: malformedTargetBefore
      .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
      .get()?.count,
    provenance: malformedTargetBefore
      .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
      .get()?.count,
    current: malformedTargetBefore
      .prepare(
        "SELECT value_text, origin, user_assertion_id, projection_commit_id FROM current_transaction_fields WHERE transaction_id = ? AND field_name = 'display_name'",
      )
      .get(Buffer.from(transactionId.replaceAll("-", ""), "hex")),
    projection: malformedTargetBefore
      .prepare("SELECT generation, commit_id FROM current_projection_state")
      .all(),
  };
  ...
  for (const [label, target] of [
    ...
    [
      "forbidden kind",
      { kind: "balance", id: transactionId, field: "display_name" },
    ],
    ["missing id", { kind: "transaction", field: "display_name" }],
    [
      "conflicting transaction target",
      { kind: "transaction", id: otherTransactionId, field: "display_name" },
    ],
  ] as const) {
    await assert.rejects(
      () =>
        commitCathayUserAssertion(derivedDir, {
          transactionId,
          target,
          field: "display_name",
          value: "forbidden",
    ...
    try {
      assert.deepEqual(
        {
          commits: malformedTargetAfter
            .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
            .get()?.count,
          userAssertions: malformedTargetAfter
            .prepare(
              "SELECT COUNT(*) AS count FROM assertions WHERE origin = 'user'",
            )
            .get()?.count,
          transitions: malformedTargetAfter
            .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
            .get()?.count,
          provenance: malformedTargetAfter
            .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
            .get()?.count,
          current: malformedTargetAfter
            .prepare(
              "SELECT value_text, origin, user_assertion_id, projection_commit_id FROM current_transaction_fields WHERE transaction_id = ? AND field_name = 'display_name'",
            )
            .get(Buffer.from(transactionId.replaceAll("-", ""), "hex")),
          projection: malformedTargetAfter
            .prepare(
              "SELECT generation, commit_id FROM current_projection_state",
            )
            .all(),
        },
        malformedTargetSnapshot,
        label,
      );
    } finally {
      malformedTargetAfter.close();
    }
  }
  ...
  const emptyScopeSnapshot = {
    commits: emptyScopeBefore
      .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
      .get()?.count,
    runs: emptyScopeBefore
      .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
      .get()?.count,
    assertions: emptyScopeBefore
      .prepare("SELECT COUNT(*) AS count FROM assertions")
      .get()?.count,
    transitions: emptyScopeBefore
      .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
      .get()?.count,
    provenance: emptyScopeBefore
      .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
      .get()?.count,
    projection: emptyScopeBefore
      .prepare("SELECT generation, commit_id FROM current_projection_state")
      .all(),
  };
  emptyScopeBefore.close();
  const diagnostic = await runCathayDerivedImportRun(derivedDir, {
    ...derivedInput,
    scope: [],
  });
  ...
  try {
    assert.deepEqual(
      {
        commits: diagnosticDb
          .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
          .get()?.count,
        runs: diagnosticDb
          .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
          .get()?.count,
        assertions: diagnosticDb
          .prepare("SELECT COUNT(*) AS count FROM assertions")
          .get()?.count,
        transitions: diagnosticDb
          .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
          .get()?.count,
        provenance: diagnosticDb
          .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
          .get()?.count,
        projection: diagnosticDb
          .prepare("SELECT generation, commit_id FROM current_projection_state")
          .all(),
      },
      emptyScopeSnapshot,
  ...
  const invalidMatrixSnapshot = {
    commits: invalidMatrixSnapshotDb
      .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
      .get()?.count,
    runs: invalidMatrixSnapshotDb
      .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
      .get()?.count,
    assertions: invalidMatrixSnapshotDb
      .prepare("SELECT COUNT(*) AS count FROM assertions")
      .get()?.count,
    transitions: invalidMatrixSnapshotDb
      .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
      .get()?.count,
    provenance: invalidMatrixSnapshotDb
      .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
      .get()?.count,
    current: invalidMatrixSnapshotDb
      .prepare(
        "SELECT transaction_id, field_name, value_text, origin, derived_assertion_id, user_assertion_id, projection_commit_id FROM current_transaction_fields ORDER BY transaction_id, field_name",
      )
      .all(),
    projection: invalidMatrixSnapshotDb
      .prepare("SELECT generation, commit_id FROM current_projection_state")
      .all(),
  };
  ...
  const invalidMatrixCases = [
    ...
    [
      "missing coordinate",
      {
        ...
        scope: validMatrix.filter(
          (coordinate) => coordinate.field === "display_name",
        ),
      },
    ],
    [
      "extra coordinate",
      { ...derivedInput, fields: ["display_name"], scope: validMatrix },
    ],
    [
      "duplicate coordinate",
      {
        ...derivedInput,
        fields: ["display_name"],
        scope: [validMatrix[0], validMatrix[0]],
      },
    ],
    [
      "unknown subject",
      {
        ...derivedInput,
        subjectIds: [unknownSubjectId],
        fields: ["display_name"],
        scope: [
          {
            transactionId: unknownSubjectId,
            field: "display_name",
            state: "supported",
            value: "unknown",
          },
  ...
  for (const [label, invalidInput] of invalidMatrixCases) {
    const invalidResult = await runCathayDerivedImportRun(
      derivedDir,
    ...
    try {
      assert.deepEqual(
        {
          commits: invalidAfter
            .prepare("SELECT COUNT(*) AS count FROM canonical_commits")
            .get()?.count,
          runs: invalidAfter
            .prepare("SELECT COUNT(*) AS count FROM derived_import_runs")
            .get()?.count,
          assertions: invalidAfter
            .prepare("SELECT COUNT(*) AS count FROM assertions")
            .get()?.count,
          transitions: invalidAfter
            .prepare("SELECT COUNT(*) AS count FROM assertion_transitions")
            .get()?.count,
          provenance: invalidAfter
            .prepare("SELECT COUNT(*) AS count FROM assertion_provenance")
            .get()?.count,
          current: invalidAfter
            .prepare(
              "SELECT transaction_id, field_name, value_text, origin, derived_assertion_id, user_assertion_id, projection_commit_id FROM current_transaction_fields ORDER BY transaction_id, field_name",
            )
            .all(),
          projection: invalidAfter
            .prepare(
              "SELECT generation, commit_id FROM current_projection_state",
            )
            .all(),
        },
        invalidMatrixSnapshot,
        label,
      );
    } finally {
      invalidAfter.close();
  ...
  await assert.rejects(
    () =>
      commitCathayUserAssertion(derivedDir, {
        target: { kind: "balance", field: "note", id: transactionId },
        value: "forbidden",
      } as never),
    /valid transaction target/,
  );
  await assert.rejects(
    () =>
      commitCathayUserAssertion(derivedDir, {
...
const blockStart =
  CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.indexOf('{"queryStatus"');
const blockEnd = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.lastIndexOf("}]}}") + 1;
const singleResultBlock = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.slice(
  blockStart,
  blockEnd,
);
for (const [label, rawResponse] of [
  [
    "zero result blocks",
    CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(`[${singleResultBlock}]`, "[]"),
  ],
  [
    "multiple result blocks",
    CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
      `[${singleResultBlock}]`,
      `[${singleResultBlock},${singleResultBlock}]`,
    ),
  ],
] as const) {
  const rejectedDir = await mkdtemp(
    join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-result-block-"),
  );
  try {
    assert.throws(
      () =>
        commitCathayDomesticDeposit(rejectedDir, {
          ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
          rawResponse,
        }),
...
const emptyDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-empty-"),
);
try {
  const emptyRaw = CATHAY_DOMESTIC_DEPOSIT_RAW_FIXTURE.replace(
    /"count":3,"startDate":"2025-08-17","endDate":"2026-08-17","details":\[[\s\S]*?\]\}/,
  ...
  const empty = await commitCathayDomesticDeposit(emptyDir, {
    ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
  ...
  try {
    ...
    assert.equal(
      emptyDb.prepare("SELECT COUNT(*) AS count FROM source_sync_states").get()
        ?.count,
      1,
    );
    assert.equal(
    ...
    for (const table of [
      ...
      assert.equal(
        emptyDb.prepare(`SELECT COUNT(*) AS count FROM ${table}`).get()?.count,
        0,
        table,
      );
    }
...
const completenessDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-completeness-"),
);
try {
  ...
  try {
    assert.equal(
      completenessDb.prepare("SELECT completeness FROM source_captures").get()
        ?.completeness,
      "complete-range",
    );
  } finally {
    completenessDb.close();
  }
} finally {
  await rm(completenessDir, { recursive: true, force: true });
}

const backfillDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-backfill-"),
);
try {
  const backfill = await commitCathayDomesticDeposit(
    backfillDir,
    {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
  ...
  try {
    const knowledge = backfillDb
      ...
      .get()?.recorded_at_utc_us as number;
    assert.equal(
      knowledge > new Date("2020-01-01T00:00:00Z").getTime() * 1000,
      true,
    );
    assert.equal(
      backfillDb
        .prepare("SELECT recorded_at_utc_us FROM canonical_commits")
        .get()?.recorded_at_utc_us,
      1787227200123456,
    );
  } finally {
    backfillDb.close();
  ...
  assert.equal(
    (
      await backfillQuery.historical({
        kind: "historical",
        cutoff: { kind: "both", financialAt: "2026-12-31", knowledgeAt: "0" },
      })
    ).transactions.length,
    0,
  );
    ...
    (
      await backfillQuery.historical({
        ...
        cutoff: {
          kind: "both",
          financialAt: "2026-12-31",
          knowledgeAt: String(backfill.commitSequence),
        },
      })
    ).transactions.length,
    3,
  ...
  await rm(backfillDir, { recursive: true, force: true });
}

const contendedDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-writer-"),
);
try {
  await Promise.all([
    commitCathayDomesticDeposit(contendedDir, CATHAY_DOMESTIC_DEPOSIT_FIXTURE),
    commitCathayDomesticDeposit(contendedDir, {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
      observedAt: "2026-08-18T00:00:00+08:00",
    }),
  ]);
  ...
  await rm(contendedDir, { recursive: true, force: true });
}

const newerSchemaDir = await mkdtemp(
...
try {
  ...
  await rm(newerSchemaDir, { recursive: true, force: true });
}
...
for (const version of [1, 2] as const) {
  const migrationDir = await mkdtemp(
    join(
      process.env.TMPDIR ?? "/tmp",
      `cathay-canonical-migration-v${version}-`,
    ),
  );
  try {
    ...
    try {
      ...
      assert.equal(
        migrated.prepare("SELECT COUNT(*) AS count FROM source_records").get()
          ?.count,
        1,
        `v${version} source row`,
      );
      assert.equal(
        migrated
          .prepare("SELECT COUNT(*) AS count FROM transaction_revisions")
      ...
      assert.equal(
        migrated.prepare("SELECT completeness_basis FROM source_captures").get()
          ?.completeness_basis,
        "success-status-scope-count-details",
        `v${version} completeness`,
      );
      assert.equal(
        migrated.prepare("SELECT cursor FROM source_sync_states").get()?.cursor,
        "legacy-cursor",
        `v${version} cursor preservation`,
      );
      assert.equal(
        migrated
          .prepare(
          ...
          .get()?.count,
        2,
        `v${version} time lineage`,
      );
      assert.equal(
        migrated
          .prepare(
            "SELECT 1 FROM sqlite_master WHERE type = 'index' AND name = 'idx_canonical_commits_sequence'",
          )
          .get()?.["1"],
        1,
        `v${version} commit sequence index`,
      );
      assert.equal(
        migrated
          .prepare(
    ...
    assert.equal(
      (await migratedQuery.current({ kind: "current" })).transactions.length,
      1,
      `v${version} current query`,
    );
    assert.equal(
      (
        await migratedQuery.historical({
          kind: "historical",
          cutoff: { kind: "both", financialAt: "2026-12-31", knowledgeAt: "1" },
        })
      ).transactions.length,
      1,
      `v${version} historical query`,
    );
    const migratedLineage = await migratedQuery.lineage({
      kind: "lineage",
      subject: {
        kind: "transaction",
    ...
    assert.equal(
      migratedLineage.entries.length,
      1,
      `v${version} lineage query`,
    );
  } finally {
...
// All pending migrations share one outer transaction. A deliberately broken
// sync relation therefore rolls the v1->v2 attempt back to the original v1
// shape; repairing the relation then permits the complete chain to retry.
const v1RollbackDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-migration-v1-rollback-"),
);
try {
  ...
  try {
    ...
    assert.equal(
      afterV1Failure
        .prepare(
          "SELECT 1 FROM sqlite_master WHERE name IN ('assertions', 'assertion_transitions', 'derived_import_runs')",
        )
        .get(),
      undefined,
    );
    const v2CaptureColumns = afterV1Failure
      .prepare("PRAGMA table_info(source_captures)")
      .all() as Array<{ name?: string }>;
    assert.equal(
      v2CaptureColumns.some((column) => column.name === "completeness_basis"),
      false,
    );
  ...
  await rm(v1RollbackDir, { recursive: true, force: true });
}
...
const v2RollbackDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-migration-v2-rollback-"),
);
try {
  ...
  try {
    ...
    const v2CaptureColumns = afterV2Failure
      .prepare("PRAGMA table_info(source_captures)")
      .all() as Array<{ name?: string }>;
    assert.equal(
      v2CaptureColumns.some((column) => column.name === "completeness_basis"),
  ...
  try {
    ...
    assert.equal(
      retriedV2.prepare("SELECT COUNT(*) AS count FROM source_records").get()
        ?.count,
      1,
    );
  } finally {
  ...
  await rm(v2RollbackDir, { recursive: true, force: true });
}

const v3MigrationDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-migration-v3-"),
);
try {
  ...
  try {
    ...
    assert.equal(
      migratedV3.prepare("SELECT COUNT(*) AS count FROM source_records").get()
        ?.count,
      3,
    );
    assert.equal(
    ...
    assert.equal(
      migratedV3
        .prepare("SELECT 1 FROM sqlite_master WHERE name = 'capture_scopes'")
        .get()?.["1"],
      1,
    );
  } finally {
  ...
  const v3RollbackDir = await mkdtemp(
    join(
      process.env.TMPDIR ?? "/tmp",
      "cathay-canonical-migration-v3-rollback-",
    ),
  );
  try {
    await commitCathayDomesticDeposit(
    ...
    await rm(v3RollbackDir, { recursive: true, force: true });
  }
} finally {
  await rm(v3MigrationDir, { recursive: true, force: true });
}

const populatedV5MigrationDir = await mkdtemp(
  join(
    process.env.TMPDIR ?? "/tmp",
    "cathay-canonical-migration-populated-v5-",
  ),
);
try {
  ...
  try {
    assert.equal(
      Number(genuineV5.prepare("PRAGMA user_version").get()?.user_version),
      5,
    );
    assert.equal(
      genuineV5
    ...
    assert.equal(
      genuineV5
        .prepare(
          "SELECT type FROM sqlite_master WHERE name = 'source_assertions'",
        )
        .get()?.type,
      "table",
    );
    assert.equal(
      genuineV5
        .prepare(
          "SELECT type FROM sqlite_master WHERE name = 'assertion_lifecycle_events'",
        )
        .get()?.type,
      "table",
    );
  ...
  try {
    assert.equal(
      Number(
        failedPopulatedV5.prepare("PRAGMA user_version").get()?.user_version,
      ),
      5,
    );
    assert.equal(
    ...
    assert.equal(
      failedPopulatedV5
        .prepare("SELECT COUNT(*) AS count FROM assertion_lifecycle_events")
        .get()?.count,
      1,
    );
    assert.equal(
  ...
  try {
    assert.equal(
      Number(
        retriedPopulatedV5.prepare("PRAGMA user_version").get()?.user_version,
      ),
      CANONICAL_SCHEMA_VERSION,
    );
    assert.equal(
    ...
    retriedPopulatedV5.close();
  }
  const migratedV5Query = createCathayCanonicalFinancialQuery(
    populatedV5MigrationDir,
  );
  assert.equal(
    (await migratedV5Query.current({ kind: "current" })).transactions.length,
    1,
  );
  assert.equal(
    (
      await migratedV5Query.historical({
        kind: "historical",
        cutoff: { kind: "both", financialAt: "2026-12-31", knowledgeAt: "1" },
      })
    ).transactions.length,
  ...
  await rm(populatedV5MigrationDir, { recursive: true, force: true });
}

const corruptDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-corrupt-"),
);
try {
  await commitCathayDomesticDeposit(
    corruptDir,
...
const malformedV5Dir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-malformed-v5-"),
);
try {
  await commitCathayDomesticDeposit(
    malformedV5Dir,
  ...
  await rm(malformedV5Dir, { recursive: true, force: true });
}

const malformedV6SpineDir = await mkdtemp(
  join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-malformed-v6-spine-"),
);
try {
  await commitCathayDomesticDeposit(
    malformedV6SpineDir,
  ...
  await rm(malformedV6SpineDir, { recursive: true, force: true });
}
...
for (const [label, capture] of [
  ...
  [
    ...
    {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
      rawResponse: CATHAY_DOMESTIC_DEPOSIT_FIXTURE.rawResponse.replace(
        '"returnCode":"0000"',
        '"returnCode":"000"',
      ),
    },
  ...
  [
    "invalid local date time",
    {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
      rawResponse: CATHAY_DOMESTIC_DEPOSIT_FIXTURE.rawResponse.replace(
        '"txnDateTime":"2026-07-01T09:00:00"',
        '"txnDateTime":"2026-02-30T09:00:00"',
      ),
    },
  ],
  [
    "scope mismatch",
    {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
      scope: {
        ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE.scope,
        endDate: "2026-08-16",
      },
    },
  ],
  [
    ...
    {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
      authorityRoute: "cathay/domestic-deposit/other",
    },
  ],
  [
    "non-exact amount",
  ...
  [
    "unsupported posting fields",
    {
      ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE,
      rawResponse: CATHAY_DOMESTIC_DEPOSIT_FIXTURE.rawResponse.replace(
        '"description":"Synthetic Cathay deposit description"',
        '"status":"pending","description":"Synthetic Cathay deposit description"',
      ),
    },
  ],
  [
  ...
  [
    "timezone-less observedAt",
    { ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE, observedAt: "2026-08-17T12:00:00" },
  ],
  [
    "unsafe timestamp microseconds",
    { ...CATHAY_DOMESTIC_DEPOSIT_FIXTURE, observedAt: "3000-08-17T12:00:00Z" },
  ],
] as const) {
  const rejectedDir = await mkdtemp(
    join(process.env.TMPDIR ?? "/tmp", "cathay-canonical-reject-"),
  );
  try {
    assert.throws(
      () => commitCathayDomesticDeposit(rejectedDir, capture),
      /./,
      label,
    );
    const rejectedDb = openCanonicalDatabase(rejectedDir);

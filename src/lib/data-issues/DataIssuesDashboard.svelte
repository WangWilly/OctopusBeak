<script lang="ts">
  ...
  async function load() {
    ...
    try {
      if (issueId) state = { status: "detail", issue: await window.octopusBeak.dataIssues.load(issueId), preview: null };
      else state = { status: "list", issues: await window.octopusBeak.dataIssues.list() };
    } catch (error) {
  ...
  async function refreshDetail(dataIssueId: string, preview: ExclusionPreviewDto | null) {
    try {
      state = { status: "detail", issue: await window.octopusBeak.dataIssues.load(dataIssueId), preview };
    } catch {
...
>
  <div class="content data-issues-content">
    ...
    {:else if state.status === "error"}
      <section class="workflow-card card" role="alert">
        <div class="stage-error"><AlertTriangle size={20} aria-hidden="true" /><span>{state.message}</span></div>
          ...
          {#if restorePreview}
            <div class="stage-reveal" transition:slide={stageTransition}>
              <div class="workflow-step active restore-step"><span class="step-mark">4</span><strong>{$t.dataIssues.restoreImport}</strong>
                {#if !restorePreview.allowed}<div class="stage-error"><AlertTriangle size={18} aria-hidden="true" /><span>{$t.dataIssues.restoreBlocked}<small>{restorePreview.blockedBy.map((item) => item.updatedAt).join(" · ")}</small></span></div>{:else}<div class="step-actions"><button class="button primary" disabled={busy} onclick={confirmRestore}>{$t.dataIssues.confirmRestore}</button></div>{/if}
              </div>
            </div>
          {/if}
        {/if}

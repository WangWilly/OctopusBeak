...
const runTaskSource = source.slice(
  source.indexOf("async function runTask"),
  source.indexOf("async function runParallelTasks"),
);
const runParallelTasksSource = source.slice(
  source.indexOf("async function runParallelTasks"),
...
assert.match(
  source,
  /\$: iconTasks = automation\.tasks\.filter\([\s\S]*?task\.status === "failed"/,
);
...
assert.match(
  source,
  /\.active-task-filter\s*\{[\s\S]*?border: 1px solid var\(--border\)/,
);
...
const openCredentialsSource = source.slice(
  source.indexOf("function openCredentials"),
  source.indexOf("function closeCredentials"),
);
assert.match(
...
assert.match(
  source,
  /\$: selectedCredentialGroup =\s+visibleCredentialGroups\.find/,
);
...
assert.match(
  saveCredentialsSource,
  /catch \{[\s\S]*?actionError = \$t\.automation\.saveCredentialsFailed/,
);
assert.doesNotMatch(
  saveCredentialsSource,
...
assert.match(
  source,
  /certificateFileValidationMessage\(storedFileError, true\)/,
);
assert.match(
  source,
...
assert.doesNotMatch(
  saveCredentialsSource.slice(
    saveCredentialsSource.indexOf("if (invalid)"),
    saveCredentialsSource.indexOf("const updates"),
  ),
